...
# load configuration
config = ConfigParser()
config.read('starlab.cfg')
dataset_config = config['DOGS']
...
ADJ_PATH = config['DOGS']['ADJ_PATH']
# load the data set.
dataset = JediDataset('dog', features_path=dataset_config['DATA_PATH'])

model = joblib.load(config['DOGS']['MODEL_PATH'])
...
try:
  fire_db = db.reference('dog/anelakur')
except:
...
def home():
  if request.args.get('email'):
    ...
    return redirect(url_for('start'))
  return render_template('home.html', task='https://www.nationalgeographic.com/content/dam/animals/thumbs/rights-exempt/mammals/d/domestic-dog_thumb.ngsversion.1546554600360.adapt.1900.1.jpg')
...
def start():
  ...
  if request.method == 'POST':
    ...
    labeled_images_ref = db.reference('dog/{}/labeled_images'.format(session['ehash']))
    labeled_images = labeled_images_ref.get()
    if not labeled_images:
      curr_img_id = np.argmin(I)
  ...
  if labeled_images:
    ...
    session['is_new'] = 'y';
    _ref = db.reference('dog/{}/'.format(session['ehash']))
...
def screen_one(curr_img_id, ctr):
  ...
  if request.method == 'POST':
    ...
    _ref = db.reference('dog/{}/step1/'.format(session['ehash']))
    class_labels = _ref.get()
    if class_labels:
      class_labels[int(curr_img_id)] = class_label
    else:
      class_labels = {curr_img_id: class_label, 515: 'None'}
...
def screen_two(curr_img_id, ctr, lbl):
  ...
  if request.method == 'POST':
    ...
    _ref = db.reference('dog/{}/step2'.format(session['ehash']))
    class_exp_labels = _ref.get()
    if not class_exp_labels:
      class_exp_labels = {curr_img_id: {'class_label': class_label}, 515:{'class_label': 'None'}}
    else:
...
# add to labeled images.
    ...
    yl_ref = db.reference('dog/{}/yl'.format(session['ehash']))
    yl_prob_ref = db.reference('dog/{}/yl_prob'.format(session['ehash']))
...
def screen_three(curr_img_id, ctr, mask, choosen_class):
  ...
  if request.method == 'POST':
    ...
    _ref = db.reference('dog/{}/step3'.format(session['ehash']))
...
if __name__ == '__main__':
  app.run(host='10.218.104.19', port=5001)
